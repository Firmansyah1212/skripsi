...
import com.example.firmanskripsi.alg.LSB2bit;
import com.example.firmanskripsi.dialog.MobiProgressBar;
import com.example.firmanskripsi.handler.ProgressHandler;
import com.example.firmanskripsi.mms.MmsIntent;
...
public class EncodeActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		/*Button buttonSelectContact = (Button) findViewById(R.id.ButtonContact);
		buttonSelectContact.setOnClickListener(new Button.OnClickListener() {
			public void onClick(View v) {

				Intent contactPickerIntent = new Intent(Intent.ACTION_PICK,
						Contacts.CONTENT_URI);// People.CONTENT_URI until 1.6
				...
				if (resultCode == RESULT_OK) {
					...
					try {
						if (c.moveToFirst()) {
							String name = c
									.getString(c
											.getColumnIndexOrThrow(Contacts.DISPLAY_NAME));//People.DISPLAY_NAME until 1.6
							TextView textNum = (TextView) this
									.findViewById(R.id.EditTextNum);
							textNum.setText(name);
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
